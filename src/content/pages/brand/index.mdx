---
title: "Our visual identity"
subtitle: Design Assets
icon: image
description: "Resources rooted in our brand foundation that reflect and amplify our purpose, people, and product."
slug: "brand"
meta:
  title: "Datum Brand Assets: Logos, Guidelines & Identity"
  description: "Explore the Datum brand design system: design principles, typography, color palettes, logos, and graphics for a hyperscale-ready framework."
  og:
    title: Our visual identity
    description: "Explore the Datum brand design system: principles, typography, color palettes, logos, and graphics."
    image: "../../images/og/brand.png"
---

import Container from '@components/Container.astro';
import BrandCard from '@components/brand/BrandCard.astro';

<Container tag="section" class="grid gap-6 xl:gap-9 md:grid-cols-2 lg:grid-cols-3 max-w-none content-stretch">
<BrandCard
  title="Principles"
  url="/brand/principles"
>
  <Fragment slot="image">
    ![Principles](./assets/principles.png)
  </Fragment>
</BrandCard>


<BrandCard
  title="Typography"
  url="/brand/typography"
>
  <Fragment slot="image">
    ![Typography](./assets/typography.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Color"
  url="/brand/color"
>
  <Fragment slot="image">
    ![Color](./assets/color.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Logos"
  url="/brand/logos"
>
  <Fragment slot="image">
    ![Logos](./assets/logos.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Graphics"
  url="/brand/graphics"
>
  <Fragment slot="image">
    ![Graphics](./assets/graphics.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Iconography"
  url="/brand/iconography"
>
  <Fragment slot="image">
    ![Iconography](./assets/iconography.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Social"
  url="/brand/social"
>
  <Fragment slot="image">
    ![Social](./assets/social.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="Templates"
  url="/brand/templates"
>
  <Fragment slot="image">
    ![Social](./assets/resources.png)
  </Fragment>
</BrandCard>

<BrandCard
  title="UI Guidelines"
  url="https://storybook.datum.net/"
>
  <Fragment slot="image">
    ![UI Guidelines](./assets/ui-guideline.png)
  </Fragment>
</BrandCard>

</Container>
