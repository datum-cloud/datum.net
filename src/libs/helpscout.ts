...
async function sendToHelpScout(input: HelpScoutLeadInput): Promise<void> {
  ...
  const clientId = serverEnv?.HELPSCOUT_CLIENT_ID;
  const clientSecret = serverEnv?.HELPSCOUT_CLIENT_SECRET;
  const inboxId = serverEnv?.HELPSCOUT_INBOX_ID;

  if (!clientId || !clientSecret || !inboxId) {
    throw new Error(
      'HELPSCOUT_CLIENT_ID, HELPSCOUT_CLIENT_SECRET and HELPSCOUT_INBOX_ID must be configured'
    );
  }
