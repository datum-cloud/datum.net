...
async function graphqlWithAuthToken() {
...
async function graphqlWithAppAuth() {
  ...
  const privateKey = import.meta.env.APP_PRIVATE_KEY;
  const installationId = parseInt(import.meta.env.APP_INSTALLATION_ID, 10);
...
async function issuesLabeledRoadmap(): Promise<Issues[]> {
  const graphqlWithAuth = await (import.meta.env.APP_ID || process.env.APP_ID
    ? graphqlWithAppAuth()
    : graphqlWithAuthToken());
