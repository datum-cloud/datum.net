...
import { saveJson, loadJson } from '@libs/file';
import { refreshIssues } from '@libs/postgres';
  ...
  try {
    ...
    if (projectData && projectData.issues.length > 0) {
      if (isOverOneHourAgo(projectData.updated_at)) {
        ...
        // await saveIssuesToJSON(JSON.stringify(issues));
...
async function saveIssuesToJSON(issues: string): Promise<boolean> {
...
export { getRoadmap, getProjects, saveIssuesToJSON, addVote, getVotes, getVoted };
