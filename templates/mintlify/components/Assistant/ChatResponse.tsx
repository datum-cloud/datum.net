...
const CopyButtonClassName = 'rounded-lg p-1.5 hover:bg-gray-100 cursor-pointer text-gray-500';
...
const CopyButton = ({ content }: { content: string }) => {
  ...
  return (
    <button className={CopyButtonClassName} onClick={handleCopy} aria-label="Copy response">
      {copied ? (
...
const ShimmerText = ({ children }: { children: React.ReactNode }) => (
  <>
    <span
      className="animate-shimmer bg-size-[200%_100%] bg-clip-text font-medium text-transparent"
      style={{
        backgroundImage:
...
const SearchingContainer = ({ query, children }: { query: string; children?: React.ReactNode }) => {
  ...
  return (
    <>
      <button
        className={cn(
          'group flex shrink-0 items-center gap-2.5 text-left text-gray-500 transition-colors hover:text-gray-600',
      ...
      >
        {!isSourcesOpen && (
          <span className="shrink-0 group-hover:hidden">
            <Icon icon="search" iconLibrary="lucide" size={12} color="dimgray" />
          </span>
        )}
        {hasResults && (
          <span
            className={cn('shrink-0', isSourcesOpen ? 'rotate-90' : 'hidden group-hover:block')}
          >
            <Icon icon="chevron-right" iconLibrary="lucide" color="dimgray" size={15} />
          </span>
        )}
      ...
      {isSourcesOpen && <div className="not-prose pt-0.5 pl-6">{children}</div>}
    </>
  );
};
...
const ChatSuggestions = ({ markdownLinks }: { markdownLinks: string }) => {
  ...
  return (
    <div className="not-prose flex flex-col gap-3">
      {links.map((link, idx) => {
        const href = isLocalUrl(link.path) ? normalizePath(link.path) : link.path;
        return (
...
const MarkdownContent = ({ text }: { text: string }) => {
  return (
    <div className="prose prose-sm max-w-none overflow-x-auto pb-1 text-base lg:text-sm [&_li]:text-base lg:[&_li]:text-sm [&_ol]:text-base lg:[&_ol]:text-sm [&_p]:text-base lg:[&_p]:text-sm [&_ul]:text-base lg:[&_ul]:text-sm">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: ({ children }) => <h1 className="text-lg font-semibold text-gray-950">{children}</h1>,
          h2: ({ children }) => (
            <h2 className="text-base font-semibold text-gray-950">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-base font-semibold text-gray-950">{children}</h3>
          ),
          h4: ({ children }) => (
            <h4 className="text-base font-semibold text-gray-950">{children}</h4>
          ),
          h5: ({ children }) => (
            <h5 className="text-base font-semibold text-gray-950">{children}</h5>
          ),
          h6: ({ children }) => (
            <h6 className="text-base font-semibold text-gray-950">{children}</h6>
          ),
          ...
          pre: ({ children, ...props }) => {
            ...
            return (
              <pre
                {...props}
                className={cn(
                  'overflow-x-auto rounded-xl border border-gray-200/70 bg-white p-3 text-sm text-gray-950',
                  props.className
                )}
              >
                {children}
...
export const ChatResponse = React.forwardRef<HTMLDivElement, ChatResponseProps>(
  ({ className, message, hasError, ...props }, ref) => {
    // isLast
    const showWrapper = useMemo(() => hasVisibleParts(message.parts), [message.parts]);
    ...
    if (message.parts && message.parts.length > 0) {
      return (
        <div
          ref={ref}
          className={cn('flex flex-col gap-4 self-stretch py-4', className)}
          {...props}
        >
            ...
            if (part.type === 'text') {
              ...
              const query =
                ...
                'query' in part.input
                  ...
                  : 'your documentation';
              const toolCallId = 'toolCallId' in part ? String(part.toolCallId) : `${index}`;
              return <SearchingContainer key={`${toolCallId}-call`} query={query} />;
            } else if (
              part.type === 'tool-search' &&
              'state' in part &&
              ...
              return (
                <div
                  ...
                  className={cn('flex flex-col gap-3', results.length === 0 && 'hidden')}
                >
                  <SearchingContainer query={query}>
                    <div className="flex flex-col gap-1">
                      {results.map((result: SearchResult, idx: number) => (
                        <a key={idx} href={normalizePath(result.path)} className="block py-1">
                          <span
    ...
    return (
      <div ref={ref} className={cn('flex flex-col gap-4 self-stretch py-4', className)} {...props}>
