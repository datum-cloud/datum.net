...
const DEBOUNCE_DELAY_IN_MS = 5000;
...
export function SearchBar() {
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      const response = await fetch(`https://api.mintlify.com/discovery/v1/search/${SUBDOMAIN}`, {
        method: 'POST',
        headers: {
      ...
      const transformedResults: SearchResult[] = data.map(
        (item: ApiSearchResult, index: number) => {
          ...
          return {
            ...
            header: item.metadata?.title || pathSegments[pathSegments.length - 1] || 'Untitled',
  ...
  const handleSelectResult = (result: SearchResult) => {
    ...
    try {
      const newHistory = [result, ...recentSearches.filter((item) => item.id !== result.id)].slice(
        0,
        MAX_HISTORY_ITEMS
      );
  ...
  const addIconsToResults = (items: SearchResult[]): SearchResult[] => {
    return items.map((item) => {
      ...
      return {
        ...item,
        icon: <Icon icon={iconName.toString()} />,
      };
    });
  };
