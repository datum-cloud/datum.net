...
const sidebarStyles: Record<SidebarItemStyle, { base?: string; active: string; inactive: string }> =
  {
    container: {
    ...
    card: {
      ...
      inactive: 'border-gray-950/5 hover:bg-gray-600/5 text-gray-700 hover:text-gray-900',
    },
    border: {
      base: 'ml-4 border-l py-2 lg:py-1.5 w-[calc(100%-1rem)]',
      active: `border-(--primary) ${ACTIVE_TEXT}`,
      inactive: 'border-gray-950/5 hover:border-gray-950/20 text-gray-700 hover:text-gray-900',
    },
    undecorated: {
      active: `border-(--primary) ${ACTIVE_TEXT}`,
      inactive: 'border-gray-950/5 hover:border-gray-950/20 text-gray-700 hover:text-gray-950',
    },
    arrow: {
      active: `border-(--primary) ${ACTIVE_TEXT}`,
      inactive: 'border-gray-950/5 hover:border-gray-950/20 text-gray-700 hover:text-gray-950',
  ...
  return (
    <li className="relative scroll-m-4 first:scroll-m-20" data-title={title}>
      <a
        href={page.href + '/'}
        className={cn(
          'group flex cursor-pointer items-center gap-x-3 py-1.5 pr-3 pl-4 text-left',
      ...
      >
        ...
        {page.icon && (
          <span
            className={cn(
              'inline-flex h-5 w-5 items-center justify-center rounded p-0.5',
          ...
          >
            <Icon
              icon={page.icon}
              iconLibrary="lucide"
              className={cn(isActive ? 'bg-white' : 'bg-gray-600 group-hover:bg-white')}
              overrideColor
              size={12}
            />
          </span>
