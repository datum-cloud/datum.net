...
export default function TableOfContents({ headings }: { headings: TocHeading[] }) {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="toc-wrapper sticky top-38 h-[calc(100vh-9.5rem)] w-80 shrink-0 flex-col pl-10">
        <nav className="overflow-y-auto pt-2 pb-8 text-sm leading-6">
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: 'instant' })}
            className="mb-3 flex cursor-pointer items-center gap-2 font-medium text-gray-900 transition-colors hover:text-gray-600"
          >
            <LinesIcon />
